...
class RankingController extends Controller
{
    ...
    {
        $items = \DB::table('item_user')->join('items', 'item_user.item_id', '=', 'items.id')->select('items.*', \DB::raw('COUNT(*) as count_h'))->where('type', 'have')->groupBy('items.id')->orderBy('count_h', 'DESC')->take(10)->get();
